class CustomDataTypeGeonames extends CustomDataTypeWithCommonsAsPlugin
  ...
  getFacet: (opts) ->
    opts.field = @
    new CustomDataTypeGeonamesFacet(opts)
              ...
              onClick: (ev2, btn) ->
                  ...
                  if that.getCustomSchemaSettings().geonames_username?.value
                    ...
                    dataEntry_xhr.start().done((data, status, statusText) ->
                      ...
                      cdata.conceptURI = GeonamesUtil.getConceptURIFromObject data

                      # _standard & _fulltext
                      cdata._fulltext = GeonamesUtil.getFullTextFromObject data, false
                      cdata._standard = GeonamesUtil.getStandardTextFromObject that, data, cdata, false

                      # facets
                      cdata.facetTerm = GeonamesUtil.getFacetTerm data, that.getDatabaseLanguages()
                       
                      # geojson
                      geoJSON = GeonamesUtil.getGeoJSONFromGeonamesJSON data
                      if geoJSON
                        cdata.conceptGeoJSON = geoJSON
                        
                      # get ancestors from data
                      cdata.conceptAncestors = GeonamesUtil.getConceptAncestorsFromObject data
